...
@section('content_header')
    ...
    <p class="text-muted mb-0">
        КП: <a href="{{ route('proposals.show', $proposal) }}">№{{ $proposal->id }}</a>
    </p>
@endsection
...
@section('content')
    <div class="card">
        <div class="card-body">
            ...
            <form method="POST" action="{{ route('proposal-items.update', [$proposal, $proposalItem]) }}">
                @csrf
                @method('PUT')

                <div class="form-group">
                    <label>Артикул</label>
                    <input type="text" class="form-control" value="{{ ($proposalItem->item?->sku ?? '—') . ' — ' . ($proposalItem->item?->vendor?->name ?? 'без вендора') }}" disabled>
                ...
                <div class="form-row">
                    <div class="form-group col-12 col-md-4">
                        ...
                        <input type="number" id="quantity" name="quantity" min="1"
                            ...
                            value="{{ old('quantity', $proposalItem->quantity) }}" required>
                        @error('quantity')
