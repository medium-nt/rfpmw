        ...
        <div class="card-body">
            ...
            @if (empty($employedPeople))
                <div class="alert alert-warning">
                    У этого контрагента ещё нет сотрудников. Сначала добавьте контактное лицо, затем создайте событие.
                </div>
            @endif

            <form method="POST" action="{{ route('events.store', $contractor) }}">
                @csrf
                        ...
                        @if (! empty($entities['projects']))
                            <optgroup label="Проекты">
                                @foreach ($entities['projects'] as $id => $label)
                                    <option value="project:{{ $id }}" @selected(old('link') === "project:$id")>{{ $label }}</option>
                                @endforeach
                            </optgroup>
                        @endif
                        @if (! empty($entities['requests']))
                            <optgroup label="Запросы">
                                @foreach ($entities['requests'] as $id => $label)
                                    <option value="request:{{ $id }}" @selected(old('link') === "request:$id")>{{ $label }}</option>
                                @endforeach
                            </optgroup>
                        @endif
                        @if (! empty($entities['proposals']))
                            <optgroup label="КП">
                                @foreach ($entities['proposals'] as $id => $label)
                                    <option value="proposal:{{ $id }}" @selected(old('link') === "proposal:$id")>{{ $label }}</option>
                                @endforeach
                            </optgroup>
                        @endif
                    </select>
