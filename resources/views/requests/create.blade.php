...
@section('content_header')
    <h1>Новый запрос</h1>
    <p class="text-muted mb-0">от контрагента «{{ $contractor->name }}»</p>
@endsection
        ...
        <div class="card-body">
            ...
            @if (empty($employedPeople))
                <div class="alert alert-warning">
                    У этого контрагента ещё нет сотрудников. Сначала добавьте контактное лицо, затем создайте запрос.
                </div>
            @endif

            <form method="POST" action="{{ route('requests.store', $contractor) }}">
                @csrf
                    ...
                    <div class="form-group col-12 col-md-6">
                        ...
                        <select id="project_id" name="project_id"
                            ...
                            @foreach ($projects as $id => $label)
                                <option value="{{ $id }}" @selected(old('project_id') == $id)>{{ $label }}</option>
                            @endforeach
                        </select>
                        @error('project_id')
                            <span class="text-danger">{{ $message }}</span>
